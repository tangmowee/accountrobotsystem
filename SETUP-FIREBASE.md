# RSC Check-in — การต่อ Firebase (ข้อมูลรวมศูนย์)

ระบบเช็คอิน/เช็คเอาท์ในไฟล์ `index` ทำงานได้ **2 โหมด** อัตโนมัติ:

| โหมด | เงื่อนไข | เก็บข้อมูลที่ไหน | ป้ายมุมขวาบน |
|------|----------|------------------|----------------|
| **LOCAL** | ยังไม่ใส่ Firebase config | localStorage (ในเบราว์เซอร์เครื่องเดียว) | 🟡 `LOCAL` |
| **CLOUD** | ใส่ Firebase config แล้ว | Firestore (รวมศูนย์ + เรียลไทม์ + ล็อกอิน) | 🟢 `CLOUD` |

ตอนนี้ค่าเริ่มต้นคือ **LOCAL** — ใช้ทดลอง/สาธิตได้ทันทีโดยไม่ต้องตั้งค่าอะไร

---

## ต้องย้ายข้อมูลไหม?

**ไม่ต้องครับ** ข้อมูลในโหมด LOCAL ตอนนี้เป็นแค่ข้อมูลตัวอย่าง (demo) ไม่ใช่ของจริง
เมื่อเปิดโหมด CLOUD ระบบจะเริ่มเก็บข้อมูลจริงใหม่บน Firestore เลย
(ถ้าอยากเก็บ demo ไว้ กด **"⬇️ ส่งออก CSV"** ในแท็บเช็คอินก่อนได้)

---

## ขั้นตอนต่อ Firebase (ครั้งเดียว ~10 นาที)

### 1) สร้างโปรเจกต์
1. เข้า https://console.firebase.google.com → **Add project** → ตั้งชื่อ เช่น `rsc-checkin`
2. เมนู **Build → Firestore Database → Create database** → เลือก **Production mode** → ภูมิภาค `asia-southeast1` (สิงคโปร์ ใกล้ไทยสุด)

### 2) เปิดการล็อกอิน
- เมนู **Build → Authentication → Get started → Email/Password → Enable → Save**

### 3) สร้างบัญชีพนักงาน (ล็อกอินด้วยชื่อเล่น/รหัส — ไม่ต้องใช้อีเมลจริง)
พนักงานล็อกอินด้วย **ชื่อผู้ใช้/ชื่อเล่น/รหัสพนักงาน** ระบบเติมโดเมนให้อัตโนมัติ
(ตั้งค่าโดเมนที่ `window.LOGIN_DOMAIN` ใน `public/firebase-config.js` — ค่าเริ่มต้น `rsc.local`)

- แท็บ **Authentication → Users → Add user** → ใส่อีเมลรูปแบบ **`ชื่อผู้ใช้@rsc.local`** + รหัสผ่าน
  - เช่น พนักงานชื่อเล่น "โจ้" → สร้างบัญชี `joe@rsc.local`
  - พนักงานพิมพ์แค่ **`joe`** ตอนล็อกอิน (ระบบเติม `@rsc.local` ให้เอง)
- แนะนำให้ชื่อผู้ใช้เป็น **ภาษาอังกฤษ/ตัวเลข** (เช่น `joe`, `nong`, `eng014`) — ชื่อไทยเต็มไปใส่ใน **ทะเบียนพนักงาน** (แท็บสรุปชั่วโมง) เพื่อแสดงผลและจัดกลุ่มตามแผนก
- ในทะเบียนพนักงาน ช่อง "ชื่อผู้ใช้" ให้กรอก **`joe`** (ให้ตรงกับที่ใช้ล็อกอิน)

### 4) เอา config มาใส่ในไฟล์
1. เมนู **Project settings (⚙️) → General → Your apps → Web app (`</>`)** → ตั้งชื่อ → Register
2. คัดลอกค่า `firebaseConfig` ที่ได้ มาวางทับในไฟล์ `index` ตรงบล็อก `FIREBASE_CONFIG`:

```js
const FIREBASE_CONFIG = {
  apiKey: "AIza...",
  authDomain: "rsc-checkin.firebaseapp.com",
  projectId: "rsc-checkin",
  storageBucket: "rsc-checkin.appspot.com",
  messagingSenderId: "1234567890",
  appId: "1:1234567890:web:abcdef"
};
```

บันทึกไฟล์แล้วเปิดใหม่ → ป้ายมุมขวาบนจะเปลี่ยนเป็น 🟢 `CLOUD` และขึ้นหน้าล็อกอิน

### 5) ตั้ง Security Rules (สำคัญ — กันคนนอกเข้าถึงข้อมูล)
เมนู **Firestore Database → Rules** วางตามนี้แล้ว **Publish**:

```
rules_version = '2';
service cloud.firestore {
  match /databases/{database}/documents {
    match /rsc/{doc} {
      allow read, write: if request.auth != null;   // เฉพาะคนที่ล็อกอินแล้ว
    }
  }
}
```

---

## สิ่งที่ได้เพิ่มในโหมด CLOUD
- **ข้อมูลรวมศูนย์** — พนักงานเช็คอินจากมือถือ/คอมเครื่องไหนก็ได้ ข้อมูลไปที่เดียวกัน
- **เรียลไทม์** — ตารางอัปเดตสดข้ามเครื่องทันที (Firestore `onSnapshot`)
- **ล็อกอินพนักงาน** — ต้องล็อกอินก่อนเช็คอิน และชื่อถูกล็อกเป็นบัญชีที่ล็อกอิน (กันเช็คอินแทนกัน)
- **Geofence** — ถ้างานใน Planner มีพิกัดไซต์ ระบบเตือนเมื่อเช็คอินห่างเกิน `GEOFENCE_RADIUS_M` (ค่าเริ่มต้น 300 ม. ปรับได้ในไฟล์)

---

## การรวมข้อมูลเดิมเข้าส่วนกลาง (ที่ทำ/บันทึกไปแล้ว)

ทุกรายการมี `id` เฉพาะตัว ระบบจึงรวมข้อมูลได้แบบ **กันซ้ำอัตโนมัติ** (รายการ id เดิมจะไม่ถูกเพิ่มซ้ำ
ถ้าซ้ำจะเก็บอันที่ข้อมูลใหม่กว่า เช่น รายการที่เช็คเอาท์แล้ว) — มีปุ่มในแท็บเช็คอิน มุมขวาของตารางประวัติ:

| ปุ่ม | ใช้ตอนไหน |
|------|-----------|
| **⬆️ รวมเครื่องนี้ขึ้น Cloud** | เคยใช้โหมด LOCAL แล้วมีข้อมูลค้างในเครื่อง → กดครั้งเดียวเพื่อดันขึ้น Firebase (ใช้ได้เมื่ออยู่โหมด CLOUD) |
| **💾 สำรอง (JSON)** | ดาวน์โหลดข้อมูลทั้งหมดเป็นไฟล์ (ไม่สูญข้อมูล ใช้ย้าย/สำรอง) |
| **📥 นำเข้า/รวม (JSON)** | เอาไฟล์ JSON จากเครื่องอื่นมารวมเข้ากับข้อมูลปัจจุบัน |

### กรณีที่พบบ่อย

**1) เคยใช้ LOCAL มาก่อน แล้วเพิ่งต่อ Firebase**
→ ต่อ Firebase (โหมดเป็น 🟢 CLOUD) → ล็อกอิน → กด **⬆️ รวมเครื่องนี้ขึ้น Cloud** ครั้งเดียว
ข้อมูลเดิมในเครื่องจะขึ้นไปรวมกับส่วนกลางโดยไม่ซ้ำ

**2) มีข้อมูลกระจายอยู่หลายเครื่อง (ต่างคนต่างใช้ LOCAL)**
→ แต่ละเครื่องกด **⬆️ รวมเครื่องนี้ขึ้น Cloud** ครั้งเดียว (ทำทีละเครื่อง)
ทุกเครื่องจะไหลไปรวมที่ Firebase เดียวกัน กันซ้ำด้วย id ให้อัตโนมัติ
(ถ้าเครื่องไหนเปิดออฟไลน์ไม่ได้ ให้กด **💾 สำรอง (JSON)** แล้วเอาไฟล์ไป **📥 นำเข้า/รวม** ที่เครื่องที่ต่อ Cloud)

**3) มีข้อมูลจาก Excel/ระบบเดิม**
→ จัดให้อยู่ในรูปแบบ JSON เดียวกับไฟล์สำรอง (โครงสร้าง `{ "log": [...], "plan": [...] }`)
แต่ละรายการไม่ต้องมี `id` ก็ได้ ระบบจะสร้างให้ แล้วกด **📥 นำเข้า/รวม (JSON)**
(แจ้งได้ถ้าต้องการให้ช่วยเขียนสคริปต์แปลงจาก Excel → JSON)

> ปลอดภัย: กดปุ่มรวม/นำเข้าซ้ำได้ ไม่ทำให้ข้อมูลเบิ้ล เพราะกันซ้ำด้วย id

---

## เปิดใช้ผ่านลิงก์เว็บ + หน้ามือถือพนักงาน (Firebase Hosting)

โครงสร้างไฟล์สำหรับ deploy อยู่ในโฟลเดอร์ `public/`:

| ไฟล์ | คือ | ลิงก์หลัง deploy |
|------|-----|------------------|
| `public/index.html` | **หน้ามือถือพนักงาน** (ปุ่มใหญ่ เช็คอิน/เอาท์ + GPS) — เป็นหน้าแรก | `https://<project>.web.app/` |
| `public/dashboard.html` | **แดชบอร์ดแอดมิน** (การเงิน + Planner + รายงาน) | `https://<project>.web.app/dashboard` |
| `public/firebase-config.js` | ใส่ Firebase config **ที่เดียว** ใช้ร่วมทั้ง 2 หน้า | — |

> ทั้ง 2 หน้าใช้ Firestore ชุดเดียวกัน (`rsc/rsc_checkin_log`, `rsc/rsc_planner`)
> ข้อมูลจึงรวมกันอัตโนมัติ — พนักงานเช็คอินจากมือถือ แล้วแอดมินเห็นในแดชบอร์ดทันที

### ขั้นตอน deploy (ทำครั้งแรก ~10 นาที)

1. ใส่ Firebase config ในไฟล์ **`public/firebase-config.js`** (จุดเดียว)
2. ติดตั้งเครื่องมือ (ถ้ายังไม่มี): `npm install -g firebase-tools`
3. ล็อกอิน: `firebase login`
4. แก้ชื่อโปรเจกต์ใน `.firebaserc` ให้ตรงกับ Project ID จริง (ค่าเริ่มต้นคือ `rsc-checkin`)
5. Deploy: `firebase deploy --only hosting`
6. เสร็จ → ได้ลิงก์ `https://<project>.web.app`
   - แจก **ลิงก์หลัก** ให้พนักงาน (หน้ามือถือ)
   - แอดมินเข้า **`/dashboard`**

### ทิปหน้ามือถือ
- ให้พนักงานกด "เพิ่มลงหน้าจอโฮม" (Add to Home Screen) เปิดใช้เหมือนแอปได้เลย
- ต้องเปิดผ่าน `https://` (Hosting ให้มาอยู่แล้ว) GPS/ล็อกอินถึงทำงานเต็มที่

---

## ทางเลือก: โฮสต์บน Netlify (ถ้า RSC ใช้ Netlify อยู่แล้ว)

**ไม่ต้องใช้ Firebase Hosting ก็ได้** — Firebase ทำหน้าที่แค่ฐานข้อมูล + ล็อกอิน (backend)
ส่วนไฟล์เว็บจะโฮสต์ที่ไหนก็ได้ รวมถึง Netlify เดิมของบริษัท

**ขั้นตอน:**
1. Deploy ขึ้น Netlify โดยตั้ง **publish directory = `public`**
   - เชื่อม GitHub repo กับ Netlify (มีไฟล์ `netlify.toml` ตั้งค่าให้แล้ว) หรือ
   - ลากโฟลเดอร์ `public` ไปวางที่หน้า Netlify (drag & drop)
2. ใส่ Firebase config ใน `public/firebase-config.js` (เหมือนเดิม)
3. **สำคัญ:** อนุญาตโดเมน Netlify ใน Firebase
   - Firebase Console → **Authentication → Settings → Authorized domains → Add domain**
   - ใส่โดเมน เช่น `rsc-checkin.netlify.app` หรือโดเมนจริงของบริษัท
   - ถ้าไม่ทำ ขั้นตอนนี้ ล็อกอินจากเว็บจะถูกบล็อก

**ผลลัพธ์:** หน้ามือถือพนักงานอยู่ที่ `https://<ชื่อ>.netlify.app/` • แดชบอร์ดแอดมินที่ `/dashboard`

> **หมายเหตุความปลอดภัย:** ไฟล์ `firebase-config.js` เปิดเผยได้ ไม่ใช่ความลับ —
> Firebase web config ออกแบบมาให้อยู่ฝั่ง client ความปลอดภัยจริงมาจาก
> (1) Firestore Security Rules และ (2) Authorized domains ที่ตั้งไว้ข้างบน
> เพิ่มความมั่นใจได้ด้วยการจำกัด API key ที่ Google Cloud Console → Credentials

---

## หมายเหตุทางเทคนิค
- ใช้ Firestore แบบ 1 เอกสารต่อ 1 คอลเลกชัน (`rsc/rsc_checkin_log`, `rsc/rsc_planner`) เก็บเป็น array
  เพียงพอสำหรับทีมขนาด SME (จำกัด 1 MB/เอกสาร ≈ หลายพันรายการ) หากในอนาคตข้อมูลโตมาก
  ค่อยปรับเป็น 1 เอกสารต่อ 1 รายการได้
- Firebase free tier (Spark plan) เพียงพอสำหรับการใช้งานปกติของบริษัท
